...
async function extractAchievementInfo(input) {
  let response = await axios.get(
    `https://steamcommunity.com/profiles/${input.steamID}/stats/${input.gameID}/?xml=1`,
  );
  let out = {
    complete: 0,
    iconComplete: "",
  ...
  if (response.status === 200) {
    try {
      ...
      achievements.forEach((achievement) => {
        ...
        if (
          achievement.elements[2].elements[0].cdata == input.achievementName
        ) {
          ...
          out.gameIcon =
            steamProfile.elements[0].elements[2].elements[4].elements[0].cdata;
          out.gameName =
            steamProfile.elements[0].elements[2].elements[1].elements[0].cdata;
        }
      });
    } catch (error) {
    ...
    if (out.complete == 0) out.complete = false;
    if (out.complete == 1) out.complete = true;
  } else {
